#define class_mc( name, public_field, private_field, public_method) \
    name( mc )\
    public_field( int, x, 0, "%d" )\
    public_field( float, y, 0, "%f" )\
    private_field( int, z, 10, "%d" )\
...
#define default_ctor_proto(descr) \
    classname(descr) pair(classname(descr), init)(void)

#define default_ctor(descr)  default_ctor_proto(descr) {\
            classname(descr) this; \
            descr(_1, _field_init, _field_init, _1)\
        return this; \
    }


#define classname(descr) descr(id, _4, _4, _1)

#define class(descr) \
    typedef struct classname(descr) {\
		descr( _1, _field, _4, _1)\
}  classname(descr);\
 default_ctor( descr )
